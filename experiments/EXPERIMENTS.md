...
| E007 | 2026-08-06 | Disable `add_safe_divisions_postlink` (`BIOHUB_OUTPUT_SAFE_DIVISIONS=0`), same 12 samples, re-score | pending | pending | n/a (CV only, not submitted) | running | Built via `make_validation_notebook.py --disable-safe-divisions`. Prediction “E003c”: `os.environ["BIOHUB_OUTPUT_SAFE_DIVISIONS"]="0"` runs before `add_safe_divisions_postlink` is ever consulted. Verdict decided by `biocell.cv.verdict()`: ships only if both folds improve over the E003 baseline (44b6 0.9349, 6bba 0.9306). |
