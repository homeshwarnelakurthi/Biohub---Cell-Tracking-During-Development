...
| E004 | 2026-08-03 | Confidence-ordered edge ids + explicit out-degree cap, patched onto the E000 baseline (`biohub-submission-v1`) | pending | pending | pending | running | Baseline writes edges in filter order and its only out-degree cap sits behind `OUTPUT_DIVISION_GEOMETRY_FILTER`, which defaults off — so the metric was truncating our links by write order. Only lever with a known sign. |
